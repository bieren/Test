...
public class B001 {
	...
	public void A01(){
		System.out.println("我是权限为可跨包调用的方法的一个动作，我所在的类是b.B001");
	}

	//权限为可跨包调用的方法，方法名前面的如果不是关键词void 比如 String 必须是一个类的名字 表示这个方法会返回的变量必须会是String 类型 如果没有变量就会报错  不能编译
		public String A02(){
			System.out.println("我是权限为可跨包调用的方法的一个动作，我所在的类是b.B001");
			return "我是返回值，如果方法名前面的关键词是void，则方法中不能出现值的返回  return '变量';但可以存在没有值的返回  return;  ";
	}
		
	//权限为可跨包调用的方法，方法名前面的如果不是关键词void 比如 int 必须是一个类的名字 表示这个方法会返回的变量必须会是int 类型 如果没有变量就会报错  不能编译   可以为自己定义的类
	public int A03(){
		System.out.println("我是权限为可跨包调用的方法的一个动作，我所在的类是b.B001");
		int asdasd = 1;
		return asdasd;
	...
	//权限为可跨包调用的方法，方法名前面的如果不是关键词void 比如 Llm01 必须是一个类的名字 表示这个方法会返回的变量必须会是Llm01 类型 如果没有变量就会报错  不能编译   可以为自己定义的类
	public Llm01 A04(){
	...
	private void B(){
		System.out.println("我是权限为可跨包调用的方法的一个动作，我所在的类是b.B001;方法名前面的关键词是void，则方法中不能出现值的返回  return '变量';但可以存在没有值的返回  return;");
		return;
	}
	...
	public B001 C(B001 b001) {

		System.out.println("输入指令1,调用全局公告变量a1");
